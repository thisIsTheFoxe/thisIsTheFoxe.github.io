...
function getNewWordWithSum(sum){
	let newWord = ""
	while(newWord.sum() != sum){
		newWord = words[Math.floor(Math.random()*500)][0].content;
		for(var key in json){
		...
		for(var i=0;i<newWord.length;i++){
			let n = parseInt(newWord[i]);
			if(!isNaN(n)) {
				if(res.indexOf(n) < 0) newWord = newWord.replaceAt(i,"%")
				else res.splice(res.indexOf(n), 1)
			}
		}
	}
	return newWord + " "; 
...
while(!pgrEnd){
	...
	ix++;
	if(cmd <= 10 && cmd > 0){
		if(cmd == 10) pgrEnd = true;
		let newWord = ""
		while(newWord.sum() != cmd){
			newWord = words[Math.floor(Math.random()*500)][0].content;
			for(var key in json){
				let allReplacements = json[key].filter((a) => a.match(/[1-9]|\w{2,}/))
				if(allReplacements.length <= 0) continue;
			...
			let res = getSummingItems(arr, cmd);
			if(!res || res.length < 1) continue;
			res = res[0]
			"01523541231554"; [1,1,0]
			for(var i=0;i<newWord.length;i++){
				let n = parseInt(newWord[i]);
				if(!isNaN(n)) {
					if(res.indexOf(n) < 0) newWord = newWord.replaceAt(i,"%")
		...
		resultSource += newWord + " ";
	}else if(cmd == 0) resultSource += words[Math.floor(Math.random()*500)][0].content + " ";
	else{
		while(cmd > 10){
